...
Elm.Dialog.make = function (_elm) {
   ...
   var display = function (_p0) {
      var _p1 = _p0;
      return _p1.open || _U.eq($Transit.getStatus(_p1.transition),
      $Transit.Exit) ? "block" : "none";
   };
   var opacity = function (_p2) {
      var _p3 = _p2;
      var _p6 = _p3.transition;
      if (_p3.open) {
            var _p4 = $Transit.getStatus(_p6);
            switch (_p4.ctor)
            {case "Exit": return 0;
               case "Enter": return $Transit.getValue(_p6);
               default: return 1;}
         } else {
            var _p5 = $Transit.getStatus(_p6);
            if (_p5.ctor === "Exit") {
                  return 1 - $Transit.getValue(_p6);
               } else {
                  return 0;
               }
         }
   };
   var footer = $Html.div(_U.list([$Html$Attributes.$class("modal-footer")]));
   var body = $Html.div(_U.list([$Html$Attributes.$class("modal-body")]));
   var closeButton = function (_p7) {
      var _p8 = _p7;
      return _U.list([A2($Html.button,
      _U.list([$Html$Attributes.$class("close")
              ,A2($Html$Attributes.attribute,"aria-label","Close")
              ,A2($Html$Events.onClick,_p8._0,_p8._1)]),
      _U.list([A2($Html.span,
      _U.list([A2($Html$Attributes.attribute,"aria-hidden","true")]),
      _U.list([$Html.text("×")]))]))]);
   };
   var header$ = F2(function (title,maybeClose) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("modal-header")]),
      A2($Basics._op["++"],
      _U.list([A2($Html.h1,
      _U.list([$Html$Attributes.$class("modal-title")]),
      _U.list([$Html.text(title)]))]),
      A2($Maybe.withDefault,
      _U.list([]),
      A2($Maybe.map,closeButton,maybeClose))));
   });
   var headerCustomClose = F3(function (title,address,action) {
      return A2(header$,
      title,
      $Maybe.Just({ctor: "_Tuple2",_0: address,_1: action}));
   });
   var headerNoClose = function (title) {
      return A2(header$,title,$Maybe.Nothing);
   };
   var view = function (_p9) {
      var _p10 = _p9;
      var _p11 = _p10;
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("modal")
              ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                               ,_0: "opacity"
                                               ,_1: $Basics.toString(opacity(_p11))}
                                              ,{ctor: "_Tuple2",_0: "display",_1: display(_p11)}]))]),
      _U.list([A2($Html.div,
              ...
              _p10.content)]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("modal-backdrop")]),
              _U.list([]))]));
   };
   ...
   var hideThenSend = F2(function (addr,action) {
      return Do(A2($Signal.send,addr,action));
   });
   var Hide = {ctor: "Hide"};
   var hide = Hide;
   ...
   var sendShow = F2(function (content,noOp) {
      return A2($Effects.map,
      function (_p13) {
         return noOp;
      },
      $Effects.task(A2($Signal.send,address,Show(content))));
   });
   var header = function (title) {
      return A2(header$,
      title,
      $Maybe.Just({ctor: "_Tuple2",_0: address,_1: hide}));
   };
   var onClickShow = function (content) {
      return A2($Html$Events.onClick,address,show(content));
   };
   var onClickHide = A2($Html$Events.onClick,address,hide);
   var onClickHideThenSend = F2(function (addr,action) {
      return A2($Html$Events.onClick,
      address,
      A2(hideThenSend,addr,action));
   });
   var emptyContent = _U.list([]);
   var update = F3(function (duration,action,model) {
      var _p14 = action;
      switch (_p14.ctor)
      {case "Show": var newModel = _U.update(model,
           {open: true,content: _p14._0});
           var timeline = A3($Transit.timeline,0,NoOp,duration);
           return A3($Transit.init,TransitAction,timeline,newModel);
         case "Hide": if (model.open) {
                 var newModel = _U.update(model,{open: false});
                 var timeline = A3($Transit.timeline,duration,NoOp,0);
                 return A3($Transit.init,TransitAction,timeline,newModel);
              } else return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         case "Do": var fx = A2($Effects.map,
           function (_p15) {
              return NoOp;
           },
           $Effects.task(_p14._0));
           var newModel = _U.update(model,
           {open: false,content: emptyContent});
   ...
   var initial = {transition: $Transit.initial
                 ,open: false
   ...
   return _elm.Dialog.values = {_op: _op
                               ...
                               ,show: show
                               ,hide: hide
                               ,hideThenSend: hideThenSend
                               ,sendShow: sendShow
                               ,update: update
                               ,view: view
                               ,header: header
                               ,headerNoClose: headerNoClose
                               ,headerCustomClose: headerCustomClose
                               ,header$: header$
                               ...
                               ,onClickHideThenSend: onClickHideThenSend
                               ,opacity: opacity
                               ,display: display};
};
...
Elm.Model.make = function (_elm) {
   ...
   var Model = F2(function (a,b) {
      return {dialog: a,counter: b};
   });
   var DialogAction = function (a) {
      return {ctor: "DialogAction",_0: a};
   };
   var Inc = {ctor: "Inc"};
   var NoOp = {ctor: "NoOp"};
...
Elm.Update.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return {ctor: "_Tuple2"
                           ,_0: model
                           ,_1: $Effects.none};
         case "Inc": var counter = model.counter + 1;
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,{counter: counter})
                  ,_1: $Effects.none};
         default: var _p1 = A3($Dialog.update,150,_p0._0,model.dialog);
           var newDialog = _p1._0;
           var dialogFx = _p1._1;
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,{dialog: newDialog})
                  ,_1: A2($Effects.map,$Model.DialogAction,dialogFx)};}
   });
   var actions = A2($Signal.map,
   $Model.DialogAction,
   $Dialog.actions);
   var initialModel = {dialog: $Dialog.initial,counter: 0};
...
Elm.View.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var modalContent = function (addr) {
      return _U.list([$Dialog.header("Confirm")
                     ,$Dialog.body(_U.list([A2($Html.p,
                     _U.list([]),
                     _U.list([$Html.text("Sure?")]))]))
                     ,$Dialog.footer(_U.list([A2($Html.a,
                                             _U.list([$Html$Attributes.$class("btn btn-default")
                                                     ,$Dialog.onClickHide]),
                                             _U.list([$Html.text("Nope")]))
                                             ,A2($Html.a,
                                             _U.list([$Html$Attributes.$class("btn btn-primary")
                                                     ,A2($Dialog.onClickHideThenSend,addr,$Model.Inc)]),
                                             _U.list([$Html.text("Sure")]))]))]);
   };
   var view = F2(function (addr,model) {
      ...
      _U.list([A2($Html.p,
              _U.list([]),
              _U.list([$Html.text($Basics.toString(model.counter))]))
              ,A2($Html.button,
              _U.list([$Dialog.onClickShow(modalContent(addr))]),
